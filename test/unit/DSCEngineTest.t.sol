...
import {MockMoreDebtDSC} from "../mocks/MockMoreDebtMSC.sol";
...
contract StableCoinEngineTest is StdCheats, Test {
    ...
    StableCoinEngine public dsce;
    MyStableCoin public dsc;
    ...
    function testRevertsIfTokenLengthDoesntMatchPriceFeeds() public {
        ...
        vm.expectRevert();
        new StableCoinEngine(tokenAddresses, feedAddresses, address(dsc));
    }
    ...
    function testRevertsIfTransferFromFails() public {
        ...
        vm.prank(owner);
        StableCoinEngine mockMsce = new StableCoinEngine(
            tokenAddresses,
            feedAddresses,
            address(mockMsc)
        );
        ...
        // Act / Assert
        vm.expectRevert(StableCoinEngine.StableCoinEngine__TransferFailed.selector);
    ...
    function testRevertsIfCollateralZero() public {
        ...
        vm.expectRevert(StableCoinEngine.StableCoinEngine__NeedsMoreThanZero.selector);
    ...
    function testRevertsWithUnapprovedCollateral() public {
        ...
        vm.startPrank(user);
        vm.expectRevert(
            abi.encodeWithSelector(StableCoinEngine.StableCoinEngine__TokenNotAllowed.selector, address(randToken))
        );
    ...
    function testCanDepositedCollateralAndGetAccountInfo() public depositedCollateral {
        (uint256 totalMscMinted, uint256 collateralValueInUsd) = dsce.getAccountInformation(user);
    ...
    function testRevertsIfMintedMscBreaksHealthFactor() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(StableCoinEngine.StableCoinEngine__BreaksHealthFactor.selector, expectedHealthFactor)
        );
        dsce.depositCollateralAndMintMsc(weth, amountCollateral, amountToMint);
        vm.stopPrank();
    }
    ...
    function testRevertsIfMintFails() public {
        ...
        vm.expectRevert(StableCoinEngine.StableCoinEngine__MintFailed.selector);
    ...
    function testMscTransferFromFailed() public {
        MockFailedTransferFrom mockMsc = new MockFailedTransferFrom();
        tokenAddresses = [address(mockMsc)];
        feedAddresses = [ethUsdPriceFeed];
        vm.prank(user);
        ...
        vm.startPrank(user);
        ERC20Mock(weth).approve(address(mockMsce), amountCollateral);
        // mockMsc.approve(address(mockMsce), amountToMint);
        mockMsce.depositCollateralAndMintMsc(weth, amountCollateral, amountToMint);
        // mockMsce._burnMsc(amountToMint, address(mockMsc), address(mockMsc));
        // vm.expectRevert(StableCoinEngine.StableCoinEngine__TransferFailed.selector);
        // vm.expectRevert();
        // vm.stopPrank();
    ...
    function testMustRedeemMoreThanZero() public depositedCollateralAndMintedMsc {
        ...
        dsc.approve(address(dsce), amountToMint);
        vm.expectRevert(StableCoinEngine.StableCoinEngine__NeedsMoreThanZero.selector);
        dsce.redeemCollateralForMsc(weth, 0, amountToMint);
        vm.stopPrank();
    }
    ...
    function testCanRedeemDepositedCollateral() public {
        ...
        dsce.depositCollateralAndMintMsc(weth, amountCollateral, amountToMint);
        dsc.approve(address(dsce), amountToMint);
        dsce.redeemCollateralForMsc(weth, amountCollateral, amountToMint);
        vm.stopPrank();
    ...
    function testMustImproveHealthFactorOnLiquidation() public {
        // Arrange - Setup
        MockMoreDebtDSC mockMsc = new MockMoreDebtDSC(ethUsdPriceFeed);
        tokenAddresses = [weth];
        feedAddresses = [ethUsdPriceFeed];
        address owner = msg.sender;
        vm.prank(owner);
    ...
    function testCantLiquidateGoodHealthFactor() public depositedCollateralAndMintedMsc {
        ...
        dsce.depositCollateralAndMintMsc(weth, collateralToCover, amountToMint);
        dsc.approve(address(dsce), amountToMint);
    ...
    function testLiquidatorTakesOnUsersDebt() public liquidated {
        (uint256 liquidatorMscMinted,) = dsce.getAccountInformation(liquidator);
        assertEq(liquidatorMscMinted, amountToMint);
    }

    function testUserHasNoMoreDebt() public liquidated {
        (uint256 userMscMinted,) = dsce.getAccountInformation(user);
    ...
    function testGetMsc() public {
        address dscAddress = dsce.getMsc();
